    ...
    def load_projects(self):
        ...
        for project in projects:
            ...
            if project.is_residential_complex and project.units:
                ...
                # Create subfolders for each unit
                self.create_unit_folders(project)
                logger.debug(f"Added residential project '{project.name}' with status '{project.status}' to the tree.")
            else:
                # Innregulering Button
                innregulering_btn = QPushButton("View DOCX")
                innregulering_btn.setToolTip("View Innregulering DOCX")
                ...
                self.tree.addTopLevelItem(project_item)
                logger.debug(f"Added non-residential project '{project.name}' with status '{project.status}' to the tree.")
    
    def create_unit_folders(self, project):
        """
        Creates subfolders for each unit within the main project folder.
        Each subfolder contains the 'Innregulering.docx' and 'Sjekkliste.docx' files.
        """
        ...
        for unit_name in project.units:
            ...
            try:
                ...
                # Copy template files into unit folder
                for file in required_files:
                    src = os.path.join(template_dir, file)
                    dest = os.path.join(unit_folder, file)
                    if os.path.exists(src):
                        shutil.copy(src, dest)
                        logger.info(f"Copied {file} to {unit_folder}")
                    else:
                        logger.warning(f"Template file {file} does not exist in {template_dir}")
            except Exception as e:
                logger.error(f"Failed to create unit folder at {unit_folder}: {e}")

    def get_project_folder(self, project):
        """
        Returns the path to the project's main folder.
        """
        folder_name = ""
        if project.main_contractor:
            folder_name = sanitize_filename(f"{project.main_contractor} - {project.name} - {project.number}") if (project.name or project.number) else f"{project.main_contractor} - Project_{project.id}"
        else:
            folder_name = sanitize_filename(f"{project.name}_{project.number}") if (project.name or project.number) else f"Project_{project.id}"
